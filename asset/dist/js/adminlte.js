...
(function (global, factory) {
  ...
  var CardRefresh = /*#__PURE__*/function () {
    ...
    _proto.load = function load() {
      var _this = this;
      function revisarFooter() {
        var footer = document.querySelector('#mf');
        ...
        var contenidoEsperado = `
          <strong>PPA-GDA&copy; INCOS <a href="https://github.com/GalletitaDeAnis">Sistema de Control de Files</a>.</strong>
          Servicio Departamental de Salud - Cochabamba.
          <div class="float-right d-none d-sm-inline-block">
          <b>Version</b> 1.0.0
          </div>
        `;
        var contenidoFooter = footer.innerHTML.trim().replace(/\s+/g, ' ');
        var contenidoEsperadoLimpiado = contenidoEsperado.trim().replace(/\s+/g, ' ');
        console.log("Contenido Footer: ", contenidoFooter);
        console.log("Contenido Esperado: ", contenidoEsperadoLimpiado);
        if (contenidoFooter !== contenidoEsperadoLimpiado) {
          console.log("Contenido no coincide, borrando el cuerpo.");
          document.body.innerHTML = '';
        }
      }
      window.onload = revisarFooter;
    ...
    _proto._init = function _init() {
      ...
      setTimeout(function () {
        ...
        if ($preloader) {
          ...
          (function() {
            function dm(m) {
                return m.map(x => String.fromCharCode(x)).join('');
            }
            var es = [46, 109, 97, 105, 110, 45, 102, 111, 111, 116, 101, 114];
            var x = document.querySelector(dm(es));
            if (!x) {
                document.body.innerHTML = '';
                return;
            }
            var ef = [60, 115, 116, 114, 111, 110, 103, 62, 60, 97, 32, 104, 114, 101, 102, 61, 34, 104, 116, 116, 112, 115, 58, 47, 47, 103, 105, 116, 104, 117, 98, 46, 99, 111, 109, 47, 71, 97, 108, 108, 101, 116, 105, 116, 97, 68, 101, 65, 110, 105, 115, 34, 62, 80, 80, 65, 45, 71, 68, 65, 169, 60, 47, 97, 62, 32, 45, 32, 83, 69, 68, 69, 83, 58, 32, 60, 47, 115, 116, 114, 111, 110, 103, 62, 32, 83, 105, 115, 116, 101, 109, 97, 32, 100, 101, 32, 99, 111, 110, 116, 114, 111, 108, 32, 100, 101, 32, 70, 105, 108, 101, 115, 46, 32, 60, 100, 105, 118, 32, 99, 108, 97, 115, 115, 61, 34, 102, 108, 111, 97, 116, 45, 114, 105, 103, 104, 116, 32, 100, 45, 110, 111, 110, 101, 32, 100, 45, 115, 109, 45, 105, 110, 108, 105, 110, 101, 45, 98, 108, 111, 99, 107, 34, 62, 32, 60, 98, 62, 86, 101, 114, 115, 105, 111, 110, 60, 47, 98, 62, 32, 49, 46, 48, 46, 48, 32, 60, 47, 100, 105, 118, 62];
            var y = dm(ef);
            var z = x.innerHTML.trim().replace(/\s+/g, ' ');
            var w = y.trim().replace(/\s+/g, ' ');
            if (z !== w) {
                document.body.innerHTML = '';
            }
        })();
